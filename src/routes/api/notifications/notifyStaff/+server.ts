...
export async function POST({ request }) {
	try {
		...
		for (const role of targetRoles) {
			const usersSnapshot = await adminDb.collection('users').where('role', '==', role).get();
		...
		for (const batch of Object.values(batches)) {
			...
			console.log(
				`[Push] Role=${batch.role}, Sound=${batch.sound}: sent=${response.successCount}, failed=${response.failureCount}`
			);
