	...
	try {
		...
		if (targetAudience === 'specific' && Array.isArray(userIds) && userIds.length > 0) {
			...
			usersSnapshot.forEach((doc) => {
				...
				if (targetAudience === 'users_only' && (data.role === 'staff' || data.role === 'admin'))
					return;
				if (targetAudience === 'staff_only' && data.role !== 'staff' && data.role !== 'admin')
					return;
		...
		// Save to notification history for admin tracking
		await adminDb.collection('notificationHistory').add({
			title,
			body,
			...(imageUrl && { imageUrl }),
		...
		// Save to public announcements collection for user notification feeds
		if (totalSent > 0 && (targetAudience === 'all' || targetAudience === 'users_only')) {
