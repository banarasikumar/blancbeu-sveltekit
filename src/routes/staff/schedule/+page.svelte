<script lang="ts">
	...
	let dailyBookings = $derived(
		$staffBookings
			.filter((b) => b.date && b.date === fmt(selectedDate))
	...
	<header class="sched-header">
		...
		<div class="date-bar">
			...
			<div
				...
				onkeydown={(e) => e.key === 'Enter' && (selectedDate = new Date())}
			>
				{#if viewMode === 'day'}
					<span class="date-big"
						>{selectedDate.toLocaleDateString('en-US', { weekday: 'long' })}</span
					>
					<span class="date-sm"
				...
				{:else}
					<span class="date-big"
						>{selectedDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span
					>
				{/if}
			</div>
			<button class="arrow-btn" onclick={() => navigate(1)}>
		...
		{#if viewMode === 'day'}
			<div class="day-chips">
				...
				{#if todayStats.completed > 0}<div class="chip chip-green">
						✓ {todayStats.completed} done · ₹{todayStats.revenue.toLocaleString()}
					</div>{/if}
			</div>
		{/if}
	</header>
		...
		<div class="day-view">
			{#if dailyBookings.length === 0}
				<div class="empty-day">
					...
					<h3>Free Day</h3>
					<p>
						No appointments on {selectedDate.toLocaleDateString('en-US', {
							weekday: 'long',
							month: 'long',
							day: 'numeric'
						})}
							...
							<div class="tl-track">
								...
								{#if isToday(selectedDate) && timeLinePos >= 0}
									...
									{#if timeLinePos >= dotPos && timeLinePos < nextPos}
										<div
											class="now-line"
											style="top:{((timeLinePos - dotPos) / (nextPos - dotPos)) * 100}%"
										></div>
									{/if}
								{/if}
								{#if bks.length > 0}
									<div class="slot-cards">
										{#each bks as bk}
											{@const dur =
												bk.servicesList?.reduce((a: number, s: any) => a + (s.duration || 30), 0) ||
												30}
											<button
												class="appt-card"
												style="--ac:{STATUS_COLOR[bk.status] || '#6366f1'};--ab:{STATUS_BG[
													bk.status
												] || 'rgba(99,102,241,0.1)'}"
											...
											>
												...
												<div class="ac-body">
													...
													<div class="ac-row2">
														<span class="ac-svc"
															>{bk.servicesList?.map((s: any) => s.name).join(', ') ||
																bk.serviceName ||
																'Service'}</span
														>
													</div>
				...
				>
					...
					{#if s.count > 0}
						<div class="wk-bar-wrap">
							<div
								class="wk-bar"
								style="height:{Math.min(100, s.count * 20)}%;background:{s.pending > 0
									? '#f59e0b'
									: s.inProgress
										? '#8b5cf6'
										: '#10b981'}"
							...
							{#if s.count > 0}
								<div class="mo-dots">
									{#each { length: Math.min(s.count, 3) } as _}
										<div
											class="mo-dot"
											style="background:{s.pending > 0 ? '#f59e0b' : '#6366f1'}"
										></div>
									{/each}
								</div>
							{/if}
...
<style>
	...
	.appt-card {
		...
		align-items: stretch;
		gap: 0;
		background: var(--ab, rgba(99, 102, 241, 0.1));
	...
	.wk-card.selected .wk-name,
	.wk-card.selected .wk-num,
	.wk-card.selected .wk-cnt {
		color: white;
	}
	.wk-top {
		display: flex;
	...
	.wk-bar {
		...
		border-radius: 6px;
		min-height: 4px;
		transition: height 0.4s var(--s-ease-spring);
	}
	.wk-info {
		display: flex;
		align-items: center;
