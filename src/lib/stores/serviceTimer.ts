...
export async function pauseTimer(booking: Booking) {
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            console.log(`[ServiceTimer] Fetched fresh data. isTimerRunning: ${freshBooking.isTimerRunning}, timerStart: ${freshBooking.timerStart}`);
            // Handle timerStart if it's a Firestore Timestamp (edge case)
            if (freshBooking.timerStart && typeof freshBooking.timerStart === 'object' && 'seconds' in (freshBooking.timerStart as any)) {
    ...
    if (typeof startTimestamp !== 'number') {
        console.warn(`[ServiceTimer] Invalid timerStart type: ${typeof startTimestamp}, Value: ${startTimestamp}`);
