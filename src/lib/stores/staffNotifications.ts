 ...
 */
export async function savePushEnabled(
	userId: string,
	appType: 'staff' | 'admin',
	enabled: boolean
): Promise<boolean> {
...
export async function requestNotificationPermission(
	userId: string,
	appType: 'staff' | 'admin' = _currentAppContext
): Promise<StaffNotificationResult> {
	console.log(
		`[StaffNotifications] Starting permission request for user: ${userId} (app: ${appType})`
	);
	_currentAppContext = appType;
	...
	try {
		...
		console.log(
			'[StaffNotifications] Token received (first 20 chars):',
